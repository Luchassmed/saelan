...
export async function getAllProjects() {
  try {
    const data = await fetchFromStrapi(
      "/projects?populate=image&sort=title:asc"
    );
...
export async function getProjectBySlug(slug) {
  try {
    const data = await fetchFromStrapi(
      `/projects?filters[slug][$eq]=${encodeURIComponent(slug)}&populate=image`
    );
...
export async function getProjectsByCategory(category) {
  try {
    const data = await fetchFromStrapi(
      `/projects?filters[category][$eq]=${encodeURIComponent(category)}&populate=image&sort=title:asc`
    );
    return data.data.map(transformProject);
  } catch (e) {
