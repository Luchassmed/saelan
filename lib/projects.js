...
export async function getAllProjects() {
  try {
    const data = await fetchFromStrapi(
      // Newest first. createdAt rather than `year` because `year` is a string
      // field that can hold ranges or words ("2023-24", "Igangværende"), which
      // would sort lexicographically. title is a tie-break so identical
      // timestamps can't shuffle between revalidations.
      "/projects?populate=images&sort[0]=createdAt:desc&sort[1]=title:asc"
    );
...
export async function getProjectsByCategory(category) {
  try {
    const data = await fetchFromStrapi(
      `/projects?filters[category][$eq]=${encodeURIComponent(category)}&populate=images&sort[0]=createdAt:desc&sort[1]=title:asc`
    );
    return data.data.map(transformProject);
  } catch (e) {
